...
def on_message(client, obj, msg):
    if(msg.topic=="UpdatedSlot"):
        print("Sub")
        mes=json.loads(str(msg.payload.decode("utf-8")))
        i=int(mes["DeleteSlot"])
        slot=np.array([i])
        np.save("DeletedSlot.npy",slot)
...
while True:
          ...
          if len(l)!=0:
              ...
              if(os.path.isfile('DeletedSlot.npy')):
                  dels=np.load("DeletedSlot.npy")
                  print(new_list)
                  print(dels[0])
                  new_list.remove(dels[0])
                  os.remove("DeletedSlot.npy")
              st=str(new_list)
              #mqttc.publish("total",total_slots)
              slotter={"slots":new_list,"total":total_slots}
              slott = json.dumps(slotter)
              mqttc.publish("available",slott)
              print(slott)
              #mqttc.publiqsh("LOCATION:",mall)
              time.sleep(0)
...
df=pd.DataFrame({'Date':date ,'Parking(s) Number':stall_no,'Status':no_of_per})
